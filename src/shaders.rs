// shaders.rs
use nalgebra_glm::{Vec3, Vec4, Mat3, Mat4, Vec2, dot};
use crate::vertex::Vertex;
use crate::color::Color;
use crate::noise; // Asegúrate de tener noise.rs copiado del proyecto 2

/// Uniforms fusionados: Matrices de cámara (Proy 1) + Variables de tiempo/semilla (Proy 2)
pub struct Uniforms {
    // --- Proyecto 1: Transformaciones ---
    ...
    // --- Proyecto 2: Procedural ---
    pub time: f32,
    pub seed: i32,
    // Parámetros para anillos (si se usan)
    pub ring_a: f32,
    pub ring_b: f32,
    pub ring_plane_xy: bool,
}

/// Datos que viajan del Vertex Shader al Fragment Shader (interpolados)
pub struct FragAttrs {
    pub obj_pos: Vec3, // Posición original en espacio de objeto (clave para que el ruido no "nade")
    pub normal: Vec3,
    pub uv: Vec2,
    pub depth: f32,
}
...
/// VERTEX SHADER HÍBRIDO
/// Usa la proyección de perspectiva correcta del Proyecto 1,
/// pero prepara los datos necesarios para el ruido del Proyecto 2.
pub fn vertex_shader(vertex: &Vertex, uniforms: &Uniforms) -> Option<Vertex> {
    ...
    // 1. Espacio de Mundo y Vista
    let world_pos = uniforms.model_matrix * position;
    let view_pos = uniforms.view_matrix * world_pos;

    // 2. Culling (Descartar lo que está detrás de la cámara)
    if view_pos.z >= 0.0 {
        return None;
    }

    // 3. Proyección (Clip Space)
    ...
    // 4. Transformación a Pantalla (NDC -> Screen)
    let ndc = clip_pos / w;
    let half_w = uniforms.screen_width * 0.5;
    let half_h = uniforms.screen_height * 0.5;

    let screen_x = ndc.x * half_w + half_w;
    let screen_y = -ndc.y * half_h + half_h; // Y invertido

    // Usamos profundidad positiva para el Z-Buffer
    let depth = -view_pos.z;

    // 5. Transformación de Normales (Solo rotación, sin escala/traslación)
    ...
    // Retornamos el vértice.
    // IMPORTANTE: 'position' sigue siendo la original del objeto para calcular el ruido correctamente.
    Some(Vertex {
...
// =========================================================================
//  ABAJO: LÓGICA DE RUIDO Y SHADERS (Idéntico a tu Proyecto 2)
// =========================================================================
...
impl Default for FlowParams {
    fn default() -> Self {
        Self {
            enabled: false, flow_scale: 2.0, strength: 0.0, time_speed: 0.0,
            jets_base_speed: 0.0, jets_frequency: 1.0, phase_amp: 0.0,
        }
    }
}
...
fn eval_bands_core(lon: f32, lat: f32, uniforms: &Uniforms, p: &NoiseParams) -> f32 {
    ...
    let turb = {
        let f = noise::fbm_perlin_3proj(
            Vec3::new(cln, lat, sln),
            p.turb_scale.max(1e-6),
            0.0, p.turb_octaves, p.turb_lacunarity, p.turb_gain,
...
impl FragmentShader for ProceduralLayerShader {
    fn shade(&self, frag: &FragAttrs, uniforms: &Uniforms) -> (Color, f32) {
        ...
        // Tinte de remolino para anillos
